...
> 在 JSX 中可以使用原生 js 中的 `map` 方法实现列表渲染
  ...
  return (
    <div>计数器
      <button
        ...
        style={{margin: '0 10px 10px',fontSize:32}}
       >{count}</button>
    </div>
  )
}
...
![image.png](assets/b06.png)

### 父子通信-子传父

![image.png](assets/b07.png)
